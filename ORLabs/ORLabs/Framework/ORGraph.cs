...
namespace ORLabs.Framework
{
    ...
    public class ORGraph : Graph<ORGraph.Edge, ORGraph.Node>
    {
        ...
        public class Node : Node<Edge>, IGraphGridSelectable
        {
            ...
            public static void setHoverNode(Node n)
            {
                if (HoveredNode != null)
                {
                    if (n != null && n.Index == HoveredNode.Index)
                    { return; }
                    revertColor(HoveredNode);
                }
                HoveredNode = n;
                if (HoveredNode != null)
                { HoveredNode.setPColor(HoverColor); }
        ...
        public class Edge : Edge<Node>
        {
            ...
            public static void setHoverEdge(Edge e)
            {
                if (HoveredLine != null)
                {
                    if (e != null && e.Index == HoveredLine.Index)
                    { return; }
                    revertColor(HoveredLine);
                }
                HoveredLine = e;
                if (HoveredLine != null)
                { HoveredLine.setPColor(HoverColor); }
    ...
    public class EdgeData
    {
        ...
        {
            Token t;
            if (!s.search(Signature1, out t))
            { Token.throwMissingSignature(Signature1); }
            ...
            if (!s.search(Signature2, out t))
            { Token.throwMissingSignature(Signature2); }
    ...
    public class MiscData
    {
        ...
        {
            Token t;
            if (!s.readUntilSignature(SigGridOptions, out t))
            { Token.throwMissingSignature(SigGridOptions); }
            if (!t.getArgVector2(0, ref GraphBI.Size)) { t.throwBadArg<Vector2>(0); }

            if (!s.readUntilSignature(SigGeoCode, out t))
            { UseMap = false; }
            else { UseMap = true; }
            if (UseMap)
            {
